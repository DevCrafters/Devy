...
export const RoleAlarmOffCommand: Command = {
  description: '역할 알림을 꺼드릴게요!',
  execute: async (sender, channel, _, data) => {
    ...
    const roleHas = sender.roles.filter(it => roles.includes(it));
    const roleNotHas = notSenderRoles.filter(it => rolesStarred.includes(it));

    await sender.removeRoles(roleHas);
    await sender.addRoles(roleNotHas);
    await channel.send(
      `<@!${sender.id}> 역할 알림 ${roleNotHas.length > 1 ? '다 ' : ''}껐어요!`
    );
  },
  namespace: '역할',
  regex: /역할 (.*) 알[람림] 꺼줘/,
  usage: '역할 (이름) 알람/알림 꺼줘'
};
